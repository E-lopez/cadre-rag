...
- [ ] **Milestone 1: Core API**
  ...
  * **Endpoints to Build:**
    ...
    - POST `/v1/create-index` -> Reads and indexes the raw JSON data located at `packages/api/app/data/cadre_kb.json`.
  ...
  * **Context Boundary:** All changes isolated inside `packages/api/app/`.
  ...
  * **Planned File Structure:**
    ...
    `app/functions/prompts.py` - Llama 3 guardrail prompt constants.
...
I want to separate the external clients into independent configuration files rather than grouping them in a single file. Update the 'Planned File Structure' section under Phase 1, Milestone 1 in plan.md to use this exact layout:
...
app/routes/query.py - POST /v1/query endpoint with the guardrail logic wrapper.

app/routes/index.py - POST /v1/create-index endpoint.

app/functions/prompts.py - Llama 3 guardrail prompt constants.

Save these file mappings directly into plan.md now.
